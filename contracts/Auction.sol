...
contract Auction {
    ...
    constructor(address _creator, uint _endTime, uint _minIncrement, uint _directBuyPrice, uint _startPrice, address _nftAddress, uint _tokenId){
        ...
        // The timestamp which marks the end of the auction (now + 30 days = 30 days from now)
        startTime = block.timestamp;
        // The timestamp which marks the start of the auction
        minIncrement = _minIncrement;
        // The minimum increment for the bid
        directBuyPrice = _directBuyPrice;
        // The price for a direct buy
    ...
    function placeBid() payable external returns (bool){
        ...
        // The auction creator can not place a bid
        ...
        address payable lastHightestBidder = payable(maxBidder);
        // The address of the last highest bidder
        uint256 lastHighestBid = maxBid;
        // The last highest bid
        maxBid = msg.value;
        // The new highest bid
        maxBidder = payable(msg.sender);
        // The address of the new highest bidder
        ...
        if (lastHighestBid != 0) {// if there is a bid
            lastHightestBidder.transfer(lastHighestBid);
        ...
        return true;
        // The bid was placed successfully
    }
    ...
    function getAuctionState() public view returns (AuctionState) {
        ...
        // If the auction is cancelled return CANCELLED
        if (isDirectBuy) return AuctionState.DIRECT_BUY;
        // If the auction is ended by a direct buy return DIRECT_BUY
